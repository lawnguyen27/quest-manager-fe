...
import { useGetMyUserMissionsQuery } from '../../store/api/mission/missionApi';
import { useGetMyUserAchievementsQuery } from '../../store/api/achievement/achievementApi';
...
const CustomerHomePage: React.FC = () => {
  ...
  const { data: missionsPage } = useGetMyUserMissionsQuery({ page: 0, size: 1 });
  const { data: achPage } = useGetMyUserAchievementsQuery({ page: 0, size: 100 });

  const points = wallet?.points ?? 0;
  const missionsJoined = missionsPage?.totalElements ?? 0;
  const achievementsDone =
    achPage?.content.filter((r) => r.status === 'COMPLETED').length ?? 0;
  const achievementsTotal = achPage?.content.length ?? 0;
  ...
  return (
    <div className="space-y-0 pb-0">
        ...
        <div className="relative flex min-h-[85vh] flex-col justify-end overflow-hidden bg-slate-950 md:justify-center">
          ...
          <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col gap-8 px-6 pb-16 pt-24 md:pb-24 md:pt-32">
            ...
            <h1 className="text-4xl font-black leading-tight tracking-tight text-white md:text-6xl lg:text-7xl">
              {t('home_scroll.hero_title', { name: user?.fullName || t('home_hub.fallback_name') })}
            </h1>
            <p className="max-w-2xl text-lg font-medium text-slate-200 md:text-xl">
              {t('home_scroll.hero_subtitle')}
            </p>
            <div className="flex flex-wrap items-center gap-4">
              <Button
                size="lg"
                className="rounded-2xl px-8 shadow-lg shadow-primary-500/30"
              ...
              <button
                type="button"
                onClick={() => scrollToId('section-stats')}
      ...
      <FullBleed id="section-stats" className="border-y border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-px bg-slate-200 sm:grid-cols-3 dark:bg-slate-700">
          <div className="flex flex-col items-center justify-center bg-white px-6 py-10 text-center dark:bg-slate-900">
            ...
            <p className="text-3xl font-black text-slate-900 dark:text-white">
              {walletLoading ? '—' : points.toLocaleString()}
            </p>
            <p className="mt-1 text-sm font-bold uppercase tracking-wider text-slate-500">
              {t('home_scroll.stat_points')}
            </p>
          </div>
          <div className="flex flex-col items-center justify-center bg-white px-6 py-10 text-center dark:bg-slate-900">
            <Target className="mb-3 h-8 w-8 text-primary-500" />
            <p className="text-3xl font-black text-slate-900 dark:text-white">{missionsJoined}</p>
            <p className="mt-1 text-sm font-bold uppercase tracking-wider text-slate-500">
              {t('home_scroll.stat_missions')}
            </p>
          </div>
          <div className="flex flex-col items-center justify-center bg-white px-6 py-10 text-center dark:bg-slate-900">
            <Award className="mb-3 h-8 w-8 text-violet-500" />
            <p className="text-3xl font-black text-slate-900 dark:text-white">
              {achievementsTotal > 0 ? `${achievementsDone}/${achievementsTotal}` : achievementsDone}
            </p>
            <p className="mt-1 text-sm font-bold uppercase tracking-wider text-slate-500">
              {t('home_scroll.stat_achievements')}
            </p>
          </div>
        </div>
      </FullBleed>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-6 py-20 md:grid-cols-2 md:gap-16 md:py-28">
          <div className="order-2 md:order-1">
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-400 leading-relaxed">
              {t('home_scroll.quests_body')}
            </p>
            <Button className="mt-8 rounded-2xl" onClick={() => navigate('/quests')}>
      ...
      <FullBleed className="bg-white dark:bg-slate-900">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-6 py-20 md:grid-cols-2 md:gap-16 md:py-28">
          ...
          <div>
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-400 leading-relaxed">
              {t('home_scroll.progress_body')}
            </p>
            <Button variant="secondary" className="mt-8 rounded-2xl" onClick={() => navigate('/history')}>
      ...
      <FullBleed className="bg-gradient-to-b from-amber-50/80 to-white dark:from-amber-950/20 dark:to-slate-950">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-6 py-20 md:grid-cols-2 md:gap-16 md:py-28">
          <div className="order-2 md:order-1">
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-400 leading-relaxed">
              {t('home_scroll.achievements_body')}
            </p>
            <Button className="mt-8 rounded-2xl bg-amber-600 hover:bg-amber-700" onClick={() => navigate('/quests?tab=achievements')}>
      ...
      <FullBleed className="bg-slate-900 text-white">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-6 py-20 md:grid-cols-2 md:gap-16 md:py-28">
          ...
          <div>
            ...
            <Button
              className="mt-8 rounded-2xl bg-violet-600 hover:bg-violet-700"
